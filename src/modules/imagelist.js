    ...
    this.container.innerHTML = images.map((img, index) => {
      ...
      return `
        <div class="image-list__item ${aspectClass} ${hoverClass}" 
          ...
          <img 
            ${this.options.lazy ? `data-src="${Security.escape(thumbSrc)}"` : `src="${Security.escape(thumbSrc)}"`}
            alt="${Security.escape(img.alt || img.title || '')}"
            class="image-list__img ${this.options.lazy ? 'lazy' : ''}"
            loading="lazy"
          >
          ${hover === 'overlay' || showTitle ? `
            <div class="image-list__overlay">
              ${showTitle && img.title ? `<span class="image-list__title">${Security.escape(img.title)}</span>` : ''}
            </div>
          ` : ''}
        </div>
      `;
        ...
        <div class="lightbox__thumbnails">
          ${images.map((img, i) => `
            <div class="lightbox__thumb ${i === this.currentIndex ? 'active' : ''}" data-index="${i}">
              <img src="${Security.escape(img.thumb || img.src)}" alt="">
            </div>
          `).join('')}
        </div>
      ` : ''}
  ...
  _render() {
    ...
    this.container.innerHTML = `
      <div class="image-compare__after" style="
        ...
        height: 100%;
      ">
        <img src="${Security.escape(after.src)}" alt="${Security.escape(after.label)}" style="width: 100%; height: 100%; object-fit: cover;">
        ${showLabels ? `<span class="image-compare__label image-compare__label--after">${Security.escape(after.label)}</span>` : ''}
      </div>
      ...
      ">
        <img src="${Security.escape(before.src)}" alt="${Security.escape(before.label)}" style="width: 100%; height: 100%; object-fit: cover;">
        ${showLabels ? `<span class="image-compare__label image-compare__label--before">${Security.escape(before.label)}</span>` : ''}
      </div>
...
export { ImageList, ImageLightbox, ImageLightbox as Lightbox, ImageCompare, LazyImage };
export default { ImageList, ImageLightbox, Lightbox: ImageLightbox, ImageCompare, LazyImage };
