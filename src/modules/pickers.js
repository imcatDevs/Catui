...
class DatePicker {
  ...
  _renderCalendar() {
    ...
    return `
      <div class="datepicker__header">
        <button type="button" class="datepicker__nav-btn" data-action="prev"><i class="material-icons-outlined">chevron_left</i></button>
        ...
        <button type="button" class="datepicker__nav-btn" data-action="next"><i class="material-icons-outlined">chevron_right</i></button>
      </div>
      <div class="datepicker__weekdays">${dayNames.map(d => `<span class="datepicker__weekday">${d}</span>`).join('')}</div>
      <div class="datepicker__days">${this._renderDays(year, month)}</div>
      <div class="datepicker__footer"><button type="button" class="datepicker__today-btn" data-action="today">오늘</button></div>
    `;
  }
  ...
  _renderYears(currentYear) {
    ...
    return `
      <div class="datepicker__header">
        <button type="button" class="datepicker__nav-btn" data-action="prevDecade"><i class="material-icons-outlined">chevron_left</i></button>
        <span class="datepicker__title">${startYear + 1} - ${endYear - 1}</span>
        <button type="button" class="datepicker__nav-btn" data-action="nextDecade"><i class="material-icons-outlined">chevron_right</i></button>
      </div>
  ...
  _renderMonths(year, monthNames) {
    ...
    return `
      <div class="datepicker__header">
        <button type="button" class="datepicker__nav-btn" data-action="prevYear"><i class="material-icons-outlined">chevron_left</i></button>
        <span class="datepicker__title" data-action="showYears">${year}년</span>
        <button type="button" class="datepicker__nav-btn" data-action="nextYear"><i class="material-icons-outlined">chevron_right</i></button>
      </div>
  ...
  open() { this.viewMode = 'days'; this.isOpen = true; this.picker.classList.add('is-visible'); this._update(); }
...
class ColorPicker {
  ...
  _init() {
    ...
    this.picker.innerHTML = `
      <div class="colorpicker__presets">${this.options.presetColors.map(c => `<span class="colorpicker__preset" data-color="${Security.escape(c)}" style="background:${Security.sanitizeCSS(c)}"></span>`).join('')}</div>
      <div class="colorpicker__custom"><input type="color" class="colorpicker__native" value="${Security.escape(this.options.defaultColor)}"></div>
    `;
    this.wrapper.appendChild(this.picker);
...
class DDay {
  ...
  _render() {
    ...
    this.element.innerHTML = `
      <div class="dday__title">${Security.escape(this.options.title)}</div>
