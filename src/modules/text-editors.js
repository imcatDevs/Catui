...
class TextareaAutosize {
   ...
   */
  static toolbarButtons = {
    clear: { icon: 'delete_outline', title: '내용 초기화' },
    copy: { icon: 'content_copy', title: '클립보드 복사' },
    undo: { icon: 'undo', title: '실행 취소' },
    redo: { icon: 'redo', title: '다시 실행' }
  };
   ...
   */
  _buildWrapper() {
    const { toolbar, showCount } = this.options;

    // 래퍼 생성
    this.wrapper = document.createElement('div');
    this.wrapper.className = 'textarea-autosize';

    // 텍스트영역을 래퍼 안으로 이동
    this.textarea.parentNode.insertBefore(this.wrapper, this.textarea);
    this.wrapper.appendChild(this.textarea);
    this.textarea.classList.add('textarea-autosize__input');

    // 툴바 (선택적)
    if (toolbar && toolbar.length > 0) {
      this.toolbarEl = document.createElement('div');
      this.toolbarEl.className = 'textarea-autosize__toolbar';
      this.toolbarEl.innerHTML = this._renderToolbar();
      this.wrapper.insertBefore(this.toolbarEl, this.textarea);
    }

    // 하단 상태바 (글자 수 등)
    if (showCount) {
      this.footerEl = document.createElement('div');
      this.footerEl.className = 'textarea-autosize__footer';
  ...
  _renderToolbar() {
    return this.options.toolbar.map(item => {
      if (item === '|') {
        return '<span class="textarea-autosize__separator"></span>';
      }

      const btn = TextareaAutosize.toolbarButtons[item];
      if (!btn) return '';

      return `
        <button type="button" class="textarea-autosize__btn" data-action="${item}" title="${btn.title}">
          <i class="material-icons-outlined">${btn.icon}</i>
        </button>
      `;
    }).join('');
  ...
  _calculateLineHeight() {
    ...
    this._minH = minRows * this._lineHeight + paddingY + borderY;
    this.textarea.style.minHeight = `${this._minH}px`;
    if (maxRows) {
      this._maxH = maxRows * this._lineHeight + paddingY + borderY;
      this.textarea.style.maxHeight = `${this._maxH}px`;
    }
  }
  ...
  _bindEvents() {
    ...
    // 포커스 상태
    this._onFocus = () => this.wrapper.classList.add('is-focused');
    this._onBlur = () => this.wrapper.classList.remove('is-focused');
  ...
  _handleAction(action) {
    switch (action) {
      case 'clear':
        ...
        break;
      case 'copy':
        navigator.clipboard.writeText(this.textarea.value).catch(() => {});
        break;
      case 'undo':
  ...
  _resize() {
    ...
    // maxRows 초과 시 스크롤 허용
    if (this._maxH && scrollH > this._maxH) {
  ...
  _updateFooter() {
    ...
    let info = `${chars}자`;
    if (maxLength > 0) {
      info = `${chars} / ${maxLength}자`;
      if (chars > maxLength) {
        this.footerEl.classList.add('is-over');
      } else {
        this.footerEl.classList.remove('is-over');
      }
    }
    info += ` · ${lines}줄`;

    this.footerEl.textContent = info;
  }
  ...
  destroy() {
    ...
    this.textarea.classList.remove('textarea-autosize__input');
