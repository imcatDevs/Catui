...
class Autocomplete {
  ...
  _renderResults(query) {
    if (!this.results.length) {
      this.dropdown.innerHTML = `<div class="autocomplete__no-results">${Security.escape(this.options.noResultsText)}</div>`;
      this._open();
      return;
    }
  ...
  _highlight(text, query) {
    const escaped = Security.escape(text);
    const escapedQuery = Security.escape(query).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
...
class MultiSelect {
  ...
  _renderTags() {
    const tagsHtml = this.selectedValues.map(val => {
      ...
      return `<span class="multiselect__tag" data-value="${Security.escape(String(val))}">${Security.escape(label)}<button type="button" class="multiselect__tag-remove" data-value="${Security.escape(String(val))}">&times;</button></span>`;
    }).join('');
  ...
  _renderDropdown(filter = '') {
    ...
    if (!availableOptions.length) {
      if (this.options.allowCreate && filter) {
        this.dropdown.innerHTML = `<div class="multiselect__option multiselect__option--create" data-value="${Security.escape(filter)}">+ "${Security.escape(filter)}" 추가</div>`;
      } else {
      ...
      this.dropdown.innerHTML = availableOptions.map(opt =>
        `<div class="multiselect__option" data-value="${Security.escape(String(opt.value))}">${Security.escape(opt.label)}</div>`
      ).join('');
    }
