  ...
  _createUI() {
    ...
    if (this.options.dropzone) {
      ...
      this.dropzone.innerHTML = `
        ...
        <p class="file-upload__text">${Security.escape(this.options.dropzoneText)}</p>
      ...
      reader.onload = (e) => {
        item.innerHTML = `
          ...
          <div class="file-upload__info">
            <span class="file-upload__name">${Security.escape(file.name)}</span>
            <span class="file-upload__size">${this._formatSize(file.size)}</span>
            ${progressHtml}
          </div>
          <button type="button" class="file-upload__remove" data-name="${Security.escape(file.name)}">&times;</button>
        `;
        bindRemoveEvent();
      };
...
class Rating {
  ...
  _render() {
    ...
    for (let i = 1; i <= this.options.max; i++) {
      ...
      html += `<span class="rating__star ${filled}" data-value="${i}" style="color: ${Security.sanitizeCSS(this.options.color)}"><i class="material-icons-outlined">${Security.escape(icon)}</i></span>`;
    }
...
class FormWizard {
  ...
  _createUI() {
      ...
      this.options.steps.forEach((step, index) => {
        html += `
          <div class="form-wizard__step" data-step="${index}">
            ...
            <div class="form-wizard__step-title">${Security.escape(step.title)}</div>
          </div>
        `;
      });
      html += '</div>';
    ...
    this.options.steps.forEach((step, index) => {
      html += `<div class="form-wizard__panel" data-panel="${index}">${Security.sanitize(step.content)}</div>`;
    });
    html += '</div>';
