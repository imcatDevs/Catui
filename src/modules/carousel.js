...
// 모듈 번들이 독립적으로 빌드되므로 임포트 대신 로컬 상수를 유지합니다.
// ============================================
...
class Carousel {
  ...
  constructor(element, options = {}) {
    ...
    this.options = Config.getFor('carousel', { ...Carousel.defaults(), ...options });
    ...
    if (slides.length === 0 && this.options.items.length > 0) {
      ...
      this.options.items.forEach((item, index) => {
        ...
        if (typeof item === 'string') {
          // 이미지 URL만 제공된 경우
          slide.innerHTML = `<img src="${Security.escape(item)}" alt="슬라이드 ${index + 1}" class="carousel__image">`;
        } else {
          // 객체로 제공된 경우
          let html = '';
          if (item.src) {
            html += `<img src="${Security.escape(item.src)}" alt="${Security.escape(item.alt || '')}" class="carousel__image">`;
          }
          if (item.content) {
            html += `<div class="carousel__content">${Security.sanitize(item.content)}</div>`;
          }
          if (item.title || item.description) {
            html += '<div class="carousel__caption">';
            if (item.title) html += `<h3 class="carousel__title">${Security.escape(item.title)}</h3>`;
            if (item.description) html += `<p class="carousel__description">${Security.escape(item.description)}</p>`;
            html += '</div>';
          }
  ...
  _goToSlide(index, animate = true) {
    ...
    if (effect === 'slide') {
      ...
      if (animate) {
        ...
        this.track.style.transition = 'none'; // CSS 기본 transition 무시
      }
      ...
      // 클론 경계 처리 (애니메이션 후 점프)
      if (animate && hasClones && (index < 0 || index >= this.totalSlides)) {
        const jumpTo = index < 0
          ? this.totalSlides + index
          : index - this.totalSlides;
        const jumpOffset = -((jumpTo + cloneOffset) * (100 / this.options.slidesToShow));

        const timerId = setTimeout(() => {
          if (!this.track) return;
          // 1) CSS class로 transition 완전 차단 (!important)
          this.track.classList.add('carousel__track--jumping');
          // 2) 강제 reflow — class가 적용된 상태 보장
          void this.track.offsetHeight;
          // 3) 실제 슬라이드 위치로 즉시 점프
          this.track.style.transform = `translateX(${jumpOffset}%)`;
          this._trackIndex = jumpTo;
          // 4) 강제 reflow — transform 변경이 transition 없이 적용됨을 보장
          void this.track.offsetHeight;
          // 5) 다음 프레임에서 jumping class 제거
          requestAnimationFrame(() => {
            if (!this.track) return;
            this.track.classList.remove('carousel__track--jumping');
            this.isAnimating = false;
          });
...
class Lightbox {
  ...
  constructor(options = {}) {
    ...
    this.options = Config.getFor('lightbox', { ...Lightbox.defaults(), ...options });
    this.currentIndex = 0;
    this.items = [];
    this.isOpen = false;
    this.lightbox = null;
