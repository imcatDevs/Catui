...
class Toast {
  ...
  static show(message, type = 'info', duration = 3000) {
    ...
    toast.innerHTML = `
      ...
      <span class="toast__message">${Security.escape(String(message))}</span>
  ...
  static clear() {
    if (Toast.container) {
      // 각 토스트의 close 버튼 이벤트 리스너를 명시적으로 정리
      const toasts = Toast.container.querySelectorAll('.toast');
      toasts.forEach(toast => {
        const closeBtn = toast.querySelector('.toast__close');
        if (closeBtn) {
          // cloneNode로 리스너 제거 (익명 함수 참조 불가하므로)
          const clean = closeBtn.cloneNode(true);
          closeBtn.parentNode?.replaceChild(clean, closeBtn);
        }
        toast.remove();
      });
    }
...
class Notification {
    ...
    if (config.actions.length) {
      actionsHtml = '<div class="notification__actions">' +
        config.actions.map((action, i) =>
          `<button class="notification__action" data-action="${i}">${Security.escape(String(action.text))}</button>`
        ).join('') + '</div>';
    }

    notification.innerHTML = `
      <div class="notification__icon">
        <i class="material-icons-outlined">${icons[config.type] || 'info'}</i>
      </div>
      <div class="notification__content">
        ${config.title ? `<div class="notification__title">${Security.escape(String(config.title))}</div>` : ''}
        <div class="notification__message">${Security.escape(String(config.message))}</div>
        ${actionsHtml}
      </div>
  ...
  static clear() {
    if (Notification.container) {
      const notifications = Notification.container.querySelectorAll('.notification');
      notifications.forEach(notification => {
        // cloneNode로 익명 리스너 제거
        const closeBtn = notification.querySelector('.notification__close');
        if (closeBtn) {
          const clean = closeBtn.cloneNode(true);
          closeBtn.parentNode?.replaceChild(clean, closeBtn);
...
class ProgressTracker {
  ...
  constructor(element, options = {}) {
    ...
    this.options = Config.getFor('progressTracker', { ...ProgressTracker.defaults(), ...options });
    ...
    const html = this.options.steps.map((step, index) => {
      ...
      return `
        <div class="progress-tracker__step ${status}" data-step="${index}">
          ...
          <div class="progress-tracker__label">${Security.escape(String(step))}</div>
        </div>
      `;
    }).join('');
  ...
  complete() {
    this.current = this.options.steps.length;
    this._render();
    this.options.onChange?.(this.current);
    this.events.emit('complete', this.current);
  }
...
class Skeleton {
  ...
  constructor(element, options = {}) {
    this.element = typeof element === 'string' ? document.querySelector(element) : element;
    if (!this.element) return;
    this.options = Config.getFor('skeleton', { ...Skeleton.defaults(), ...options });
    this._init();
  }
