...
class Gantt {
  ...
  _renderTaskList(tasks, level = 0) {
    ...
    tasks.forEach(task => {
      ...
      html += `
        <div class="gantt__task-row" data-task-id="${Security.escape(task.id)}" style="height: ${this.options.rowHeight}px;">
          <div class="gantt__task-name" style="padding-left: ${16 + indent}px;">
            ${hasChildren ? `
              <button class="gantt__collapse-btn" data-task-id="${Security.escape(task.id)}">
                <i class="material-icons-outlined">${task.collapsed ? 'chevron_right' : 'expand_more'}</i>
              </button>
            ` : '<span class="gantt__task-bullet"></span>'}
            <span class="gantt__task-title">${Security.escape(task.name)}</span>
            ${task.assignee ? `<span class="gantt__task-assignee">${Security.escape(task.assignee)}</span>` : ''}
          </div>
        </div>
      `;
  ...
  _renderTasks(tasks, level = 0) {
    ...
    tasks.forEach(task => {
      ...
      html += `
        <div class="gantt__bar-row" data-task-id="${Security.escape(task.id)}" style="height: ${this.options.rowHeight}px;">
          ${this._renderGridCells()}
          <div class="gantt__bar ${this.options.editable ? 'gantt__bar--draggable' : ''}" 
               data-task-id="${Security.escape(task.id)}"
               style="
                 left: ${left}px; 
                 width: ${width}px;
                 height: ${this.options.taskHeight}px;
                 top: ${(this.options.rowHeight - this.options.taskHeight) / 2}px;
                 background: ${Security.sanitizeCSS(statusColor)};
               ">
            ${this.options.showProgress ? `
              <div class="gantt__bar-progress" style="width: ${task.progress}%;"></div>
            ` : ''}
            <span class="gantt__bar-label">${Security.escape(task.name)}</span>
