...
class Stepper {
  ...
  _renderStep(step, index) {
    ...
    const iconContent = step.icon
      ? `<i class="material-icons-outlined">${Security.escape(step.icon)}</i>`
    ...
    return `
      <div class="stepper__step" 
        ...
        ${!isLast && connector !== 'none' ? '<div class="stepper__connector"><div class="stepper__connector-line"></div></div>' : ''}
        <div class="stepper__label">
          <span class="stepper__title">${Security.escape(step.title || `Step ${index + 1}`)}</span>
          ${step.subtitle ? `<span class="stepper__subtitle">${Security.escape(step.subtitle)}</span>` : ''}
        </div>
      </div>
    `;
  }
  ...
  _updateSteps() {
    ...
    stepEls.forEach((stepEl, index) => {
      ...
      // 커넥터 라인 색상 업데이트 (완료된 단계)
      const connectorLine = stepEl.querySelector('.stepper__connector-line');
      if (connectorLine) {
        if (this._completedSteps.has(index)) {
          connectorLine.classList.add('is-completed');
        } else {
          connectorLine.classList.remove('is-completed');
        }
      }
    });
  }
  ...
  _updateContent() {
    ...
    const rawContent = typeof step.content === 'function'
      ? step.content(this.options.currentStep)
      : step.content;
    const content = typeof rawContent === 'string' ? Security.sanitize(rawContent) : '';
      ...
      <div class="vertical-stepper__item ${index === this.options.currentStep ? 'is-active' : ''} ${this._completedSteps.has(index) ? 'is-completed' : ''}" 
        ...
        <div class="vertical-stepper__header" role="button" tabindex="0">
          <div class="vertical-stepper__indicator">
            <span class="vertical-stepper__number">${step.icon ? `<i class="material-icons-outlined">${Security.escape(step.icon)}</i>` : index + 1}</span>
            <span class="vertical-stepper__check"><i class="material-icons-outlined">check</i></span>
          </div>
          <div class="vertical-stepper__label">
            <span class="vertical-stepper__title">${Security.escape(step.title || `Step ${index + 1}`)}</span>
            ${step.subtitle ? `<span class="vertical-stepper__subtitle">${Security.escape(step.subtitle)}</span>` : ''}
          </div>
        ...
        <div class="vertical-stepper__content" ${this._expandedSteps.has(index) ? '' : 'hidden'}>
          <div class="vertical-stepper__body">
            ${step.content ? Security.sanitize(step.content) : ''}
          </div>
