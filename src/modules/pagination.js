...
class Pagination {
  ...
  constructor(container, options = {}) {
    this.options = Config.getFor('pagination', Utils.extend({}, Pagination.defaults(), options));
  ...
  _updateInfo() {
    ...
    const info = this.options.infoTemplate
      .replace('{{start}}', start)
      .replace('{{end}}', end)
      .replace('{{total}}', total)
  ...
  _handleClick(e) {
    ...
    const li = link.closest('.pagination__item');
    if (li.classList.contains('is-disabled') || li.classList.contains('is-active')) return;
...
class DataPaginator {
   ...
   */
  constructor(options = {}) {
    this.options = Config.getFor('dataPaginator', Utils.extend({}, DataPaginator.defaults(), options));
