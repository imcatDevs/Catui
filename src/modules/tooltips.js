...
class Tooltip {
  ...
  constructor(element, options = {}) {
    ...
    this.options = Config.getFor('tooltip', { ...Tooltip.defaults(), ...options });
    this.tooltip = null;
  ...
  _createTooltip() {
    ...
    if (this.options.html) {
      content.innerHTML = Security.sanitize(this.options.content);
    } else {
  ...
  setContent(content) {
    ...
    if (this.tooltip) {
      ...
      if (this.options.html) {
        contentEl.innerHTML = Security.sanitize(content);
      } else {
...
class Popover {
  ...
  constructor(element, options = {}) {
    ...
    this.element = element;
    this.options = Config.getFor('popover', { ...Popover.defaults(), ...options });
    this.popover = null;
    this.isVisible = false;
  ...
  _bindEvents() {
    ...
    // 외부 클릭 핸들러 정의 (show/hide에서 동적 등록/제거)
  ...
  _createPopover() {
    ...
    if (this.options.title) {
      html += `<div class="popover__header">
        <h4 class="popover__title">${Security.escape(this.options.title)}</h4>
        ${this.options.dismissible ? '<button class="popover__close" aria-label="닫기">&times;</button>' : ''}
      </div>`;
    }

    html += `<div class="popover__body">${Security.sanitize(this.options.content)}</div>`;
  ...
  _positionPopover() {
    ...
    // CSS 위치 클래스 적용 (arrow 위치용)
    this.popover.classList.remove('popover--top', 'popover--bottom', 'popover--left', 'popover--right');
    this.popover.classList.add(`popover--${placement}`);
  }
