...
class Dropdown {
  ...
  constructor(trigger, options = {}) {
    this.options = Config.getFor('dropdown', Utils.extend({}, Dropdown.defaults(), options));
  ...
  _createMenu() {
    ...
    this.menu.style.zIndex = String(Config.get('zIndex.dropdown') || 1000);

    // 아이템 추가
    this._renderItems();

    // body에 추가 (CSS visibility:hidden으로 숨김 상태)
  ...
  _renderItems() {
    ...
    this.options.items.forEach((item, index) => {
      if (item.divider) {
        ...
        if (item.active) {
          menuItem.classList.add('dropdown__item--active');
        }

        if (item.danger) {
          menuItem.classList.add('dropdown__item--danger');
        }
  ...
  _handleTriggerMouseLeave(e) {
    ...
    if (this.isOpen && this.menu) {
      ...
      const my = e.clientY;
      const pad = 10;
      if (mx >= menuRect.left - pad && mx <= menuRect.right + pad &&
          my >= menuRect.top - pad && my <= menuRect.bottom + pad) {
        return;
      }
    }
  ...
  show() {
    ...
    // is-visible 클래스로 CSS 전환 (opacity + visibility)
  ...
  hide() {
    ...
    // is-visible 제거 (CSS 전환으로 자연스럽게 숨김)
