...
class TypingIndicator {
  ...
  _updateDisplay() {
    ...
    const displayUsers = this._users.slice(0, maxDisplay).map(u => Security.escape(u));
...
class ConnectionStatus {
  ...
  _showOnline() {
    ...
    this._banner.innerHTML = `
      ...
      <span>${Security.escape(this.options.onlineMessage)}</span>
    `;
  ...
  _showOffline() {
    ...
    this._banner.innerHTML = `
      ...
      <span>${Security.escape(this.options.offlineMessage)}</span>
    `;
    this._banner.style.display = 'flex';
  }
