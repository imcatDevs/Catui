...
class OTPInput {
  ...
  _render() {
    ...
    if (this.options.size && this.options.size !== 'md') {
      this.container.classList.add(`otp-input--${this.options.size}`);
    }
    if (error) this.container.classList.add('otp-input--error');
    if (disabled) this.container.classList.add('otp-input--disabled');

    // password 타입이면서 form 안에 없는 경우 form 래퍼 추가
    const needsForm = mask && !this.container.closest('form');
    let html = needsForm ? '<form autocomplete="off" onsubmit="return false">' : '';

    html += '<div class="otp-input__fields">';
  ...
  _bindEvents() {
    ...
    this._onPaste = (e) => {
      ...
      chars = chars.slice(0, this.options.length);
  ...
  setSuccess() {
    this.clearError();
    this.container.classList.add('otp-input--success');
  }

  clearSuccess() {
    this.container.classList.remove('otp-input--success');
  }
...
class PinInput {
  ...
  _render() {
    ...
    if (this.options.size && this.options.size !== 'md') {
      this.container.classList.add(`pin-input--${this.options.size}`);
    }
  ...
  _bindEvents() {
    ...
    this._handlers.onPaste = (e) => {
      e.preventDefault();
      const pastedData = e.clipboardData.getData('text').trim();
      let chars = this.options.numeric
        ? pastedData.split('').filter(c => /^\d$/.test(c))
        : pastedData.split('');
      chars = chars.slice(0, this.options.length);
      ...
      const lastIndex = Math.min(chars.length, this.options.length) - 1;
      if (lastIndex >= 0) this.inputs[lastIndex].focus();
    ...
    this.inputs.forEach(input => {
      ...
      input.addEventListener('paste', this._handlers.onPaste);
  ...
  setSuccess() {
    this.clearError();
    this.container.classList.add('pin-input--success');
  }

  clearSuccess() {
    this.container.classList.remove('pin-input--success');
  }
  ...
  destroy() {
    this.inputs.forEach(input => {
      ...
      input.removeEventListener('paste', this._handlers.onPaste);
