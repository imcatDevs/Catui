...
describe('API Module', () => {
  ...
  describe('JSON 파싱 실패 처리', () => {
    it('응답이 유효하지 않은 JSON이면 fallback 메시지를 반환해야 함', async () => {
      ...
      const result = await APIUtil.get('/api/bad-json');
      // JSON 파싱 실패 시에도 결과를 반환해야 함
      expect(result).toBeDefined();
    });
  });

  describe('에러 응답 인터셉터 onRejected', () => {
    it('네트워크 에러 시 onRejected 인터셉터를 실행해야 함', async () => {
      const onRejected = vi.fn(err => ({ ...err, handled: true }));
      APIUtil.interceptors.response.use(null, onRejected);

      global.fetch = vi.fn().mockRejectedValue(new Error('Network Error'));

      const result = await APIUtil.get('/api/fail');
      expect(onRejected).toHaveBeenCalled();
    });

    it('onRejected 인터셉터에서 에러 발생 시 원래 에러 반환', async () => {
      APIUtil.interceptors.response.use(null, () => {
        throw new Error('interceptor error');
      });

      global.fetch = vi.fn().mockRejectedValue(new Error('Network Error'));

      const result = await APIUtil.get('/api/fail2');
