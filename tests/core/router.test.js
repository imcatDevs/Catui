...
describe('View Router Module', () => {
  ...
  describe('훅 에러 격리', () => {
    it('훅 핸들러 에러가 다른 훅에 영향을 주지 않아야 함', async () => {
      const spy = vi.spyOn(console, 'error').mockImplementation(() => {});
      const good = vi.fn();
      router.afterLoad(() => { throw new Error('hook error'); });
      router.afterLoad(good);
      ...
      await router.navigate('views/hook-err.html');
  ...
  describe('destroy()', () => {
    it('라우터 상태를 완전히 정리해야 함', async () => {
      ...
      expect(loading.forceHide).toHaveBeenCalled();
    });
  });
});
