...
describe('URL Module', () => {
  ...
  describe('removeParam()', () => {
    it('단일 파라미터를 제거해야 함', () => {
      ...
      delete window.location;
      window.location = { search: '?a=1&b=2&c=3', pathname: '/page' };
      URLUtil.removeParam('b');
      expect(spy).toHaveBeenCalled();
      spy.mockRestore();
    });

    it('배열로 여러 파라미터를 제거해야 함', () => {
      const spy = vi.spyOn(window.history, 'replaceState').mockImplementation(() => {});
      delete window.location;
      window.location = { search: '?a=1&b=2&c=3', pathname: '/page' };
      URLUtil.removeParam(['a', 'c']);
      expect(spy).toHaveBeenCalled();
      spy.mockRestore();
    });
  });
  ...
  describe('clearQuery()', () => {
    it('모든 쿼리 파라미터를 제거해야 함', () => {
      const spy = vi.spyOn(window.history, 'replaceState').mockImplementation(() => {});
      delete window.location;
      window.location = { search: '?a=1&b=2', pathname: '/page' };
      URLUtil.clearQuery();
