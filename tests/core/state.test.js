...
describe('State Module', () => {
  ...
  describe('엣지 케이스', () => {
    it('_trackDependencies에서 getter 에러는 무시하고 의존성을 반환해야 함', () => {
      const store = StateManager.create({ x: 1 });
      const storeInternal = store._store;
      const deps = storeInternal._trackDependencies(() => {
        throw new Error('getter 에러');
      });
      expect(Array.isArray(deps)).toBe(true);
    });

    it('watcher 콜백 에러는 다른 watcher에 영향을 주지 않아야 함', () => {
      const spy = vi.spyOn(console, 'error').mockImplementation(() => {});
      const store = StateManager.create({ val: 1 });
      const good = vi.fn();
      store.watch('val', () => { throw new Error('watcher error'); });
      store.watch('val', good);
      store.val = 2;
