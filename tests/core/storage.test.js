...
describe('Storage Module', () => {
  ...
  describe('cleanExpired 엣지 케이스', () => {
    it('JSON 파싱 실패 항목은 무시해야 함', () => {
    ...
    it('localStorage 접근 에러 시 0을 반환해야 함', () => {
      const orig = Storage.cleanExpired;
      const spy = vi.spyOn(console, 'error').mockImplementation(() => {});
      // localStorage.key()에서 에러 발생 시뮬레이션
      const origKey = localStorage.key;
      localStorage.key = () => { throw new Error('test'); };
