...
describe('Template Module', () => {
  describe('render()', () => {
    ...
    it('{{#key}} 섹션 블록 — truthy일 때 렌더링해야 함', () => {
      const html = Template.render('{{#show}}보임{{/show}}', { show: true });
      expect(html).toBe('보임');
    });

    it('{{#key}} 섹션 블록 — falsy일 때 빈 문자열이어야 함', () => {
      const html = Template.render('{{#show}}보임{{/show}}', { show: false });
      expect(html).toBe('');
    });

    it('{{^key}} 반전 블록 — falsy일 때 렌더링해야 함', () => {
      const html = Template.render('{{^show}}숨김{{/show}}', { show: false });
      expect(html).toBe('숨김');
    });

    it('{{^key}} 반전 블록 — truthy일 때 빈 문자열이어야 함', () => {
      const html = Template.render('{{^show}}숨김{{/show}}', { show: true });
      expect(html).toBe('');
    });

    it('{{#key}}+{{^key}} 조합이 동작해야 함', () => {
      const html = Template.render(
        '{{#show}}보임{{/show}}{{^show}}숨김{{/show}}',
        { show: true }
      );
      expect(html).toBe('보임');
      ...
      expect(html2).toBe('숨김');
    });

    it('{{#items}}{{.}}{{/items}} 원시값 배열 반복', () => {
      const html = Template.render(
        '{{#items}}<li>{{.}}</li>{{/items}}',
        { items: ['사과', '바나나', '딸기'] }
      );
      expect(html).toBe('<li>사과</li><li>바나나</li><li>딸기</li>');
    });

    it('{{#items}} 객체 배열 반복', () => {
      const html = Template.render(
        '{{#users}}<p>{{name}} ({{role}})</p>{{/users}}',
        { users: [{ name: '홍길동', role: 'Admin' }, { name: '김영희', role: 'User' }] }
      );
      expect(html).toBe('<p>홍길동 (Admin)</p><p>김영희 (User)</p>');
    });

    it('빈 배열 섹션은 렌더링하지 않아야 함', () => {
      const html = Template.render(
        '{{#items}}<li>{{.}}</li>{{/items}}',
        { items: [] }
      );
  ...
  describe('renderRaw()', () => {
    it('template이 문자열이 아니면 빈 문자열을 반환해야 함', () => {
      expect(Template.renderRaw(null)).toBe('');
      expect(Template.renderRaw(123)).toBe('');
      expect(Template.renderRaw(undefined)).toBe('');
    });
  });
});
