...
describe('Form Module', () => {
  ...
  describe('reset()', () => {
    it('에러 메시지 요소가 있으면 숨겨야 함', () => {
      const v = FormValidator.create(form, {
        username: { required: true }
      });
      // 에러 메시지 요소 생성
      const errEl = document.createElement('div');
      errEl.className = v.options.errorMessageClass;
      errEl.style.display = 'block';
      form.elements.username.parentElement.appendChild(errEl);
  ...
  describe('pattern 검증', () => {
    it('잘못된 정규식 패턴은 에러 로그 후 invalid 반환', () => {
      const spy = vi.spyOn(console, 'error').mockImplementation(() => {});
      const v = FormValidator.create(form, {
        username: { pattern: '[invalid' }
      });
      form.elements.username.value = 'test';
