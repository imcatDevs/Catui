...
describe('Module Loader', () => {
  ...
  describe('_toPascalCase()', () => {
    it('단순 모듈명을 파스칼케이스로 변환해야 함', () => {
      expect(loader._toPascalCase('modal')).toBe('Modal');
      expect(loader._toPascalCase('dropdown')).toBe('Dropdown');
      expect(loader._toPascalCase('m')).toBe('M');
    });

    it('케밥케이스 모듈명을 파스칼케이스로 변환해야 함', () => {
      expect(loader._toPascalCase('data-viz')).toBe('DataViz');
      expect(loader._toPascalCase('live-status')).toBe('LiveStatus');
      expect(loader._toPascalCase('advanced-ui')).toBe('AdvancedUi');
      expect(loader._toPascalCase('text-editors')).toBe('TextEditors');
      expect(loader._toPascalCase('media-viewer')).toBe('MediaViewer');
      expect(loader._toPascalCase('security-input')).toBe('SecurityInput');
    });

    it('3단어 케밥케이스도 변환해야 함', () => {
      expect(loader._toPascalCase('a-b-c')).toBe('ABC');
    });
  });
  ...
  describe('_loadModuleCSS()', () => {
    it('이미 로드된 CSS는 스킵해야 함', async () => {
      loader.loadedCSS.add(`${loader.basePath}/test.css`);
      await loader._loadModuleCSS('test');
      // 에러 없이 완료
      expect(loader.loadedCSS.has(`${loader.basePath}/test.css`)).toBe(true);
    });

    it('로딩 중인 CSS는 기존 Promise를 재사용해야 함', async () => {
      const cssPath = `${loader.basePath}/pending.css`;
      const promise = Promise.resolve();
      loader._pendingCSS.set(cssPath, promise);
      const result = await loader._loadModuleCSS('pending');
      expect(result).toBeUndefined();
    });

    it('CSS 로드 실패는 무시해야 함', async () => {
      loader.loadCSS = vi.fn().mockRejectedValue(new Error('404'));
      await loader._loadModuleCSS('nonexistent');
      // 에러 없이 완료
    });

    it('CSS 로드 성공 시 loadedCSS에 추가해야 함', async () => {
      global.requestAnimationFrame = vi.fn(cb => cb());
      loader.loadCSS = vi.fn().mockResolvedValue();
      await loader._loadModuleCSS('success-module');
      expect(loader.loadedCSS.has(`${loader.basePath}/success-module.css`)).toBe(true);
