...
describe('DOM Module', () => {
  ...
  describe('텍스트 및 HTML', () => {
    ...
    it('text() - XSS를 자동으로 이스케이프해야 함', () => {
      ...
      // textContent는 원본 문자열 그대로 반환 (HTML 인코딩 없음)
      expect(el.text()).toBe('<script>alert(1)</script>');
      // DOM의 innerHTML은 브라우저가 자동 이스케이프하므로 <script> 태그가 없어야 함
  ...
  describe('DOM.ready()', () => {
    ...
    it('readyState가 loading이면 DOMContentLoaded에 등록해야 함', () => {
      const origState = Object.getOwnPropertyDescriptor(Document.prototype, 'readyState');
      Object.defineProperty(document, 'readyState', { value: 'loading', configurable: true });
      const spy = vi.spyOn(document, 'addEventListener');
      const cb = vi.fn();
      DOM.ready(cb);
      expect(spy).toHaveBeenCalledWith('DOMContentLoaded', cb);
      expect(cb).not.toHaveBeenCalled();
      spy.mockRestore();
      if (origState) {
        Object.defineProperty(document, 'readyState', origState);
      } else {
  ...
  describe('DOM.select() 엣지 케이스', () => {
    it('DOMElement 인스턴스를 전달하면 그대로 반환해야 함', () => {
      const el = DOM.select('#parent');
      const same = DOM.select(el);
      expect(same).toBe(el);
    });

    it('지원하지 않는 타입은 빈 DOMElement 반환해야 함', () => {
      const result = DOM.select(12345);
