...
describe('Loading Indicator Module', () => {
  ...
  describe('destroy()', () => {
    it('타이머와 DOM 요소를 정리해야 함', () => {
      loading.show('정리 테스트');
      vi.advanceTimersByTime(200);
      expect(loading.isVisible()).toBe(true);

      loading.destroy();
      expect(loading.isShowing).toBe(false);
      expect(loading.element).toBeNull();
    });

    it('showTimer가 있으면 clearTimeout 해야 함', () => {
      // show()는 딜레이가 있으므로 advanceTimers 전에 destroy
      loading.show('타이머 테스트');
      loading.destroy();
      expect(loading.showTimer).toBeNull();
    });

    it('hideTimer가 있으면 clearTimeout 해야 함', () => {
      loading.show('하이드 타이머');
      vi.advanceTimersByTime(200);
      loading.hide();
      // hide도 딜레이가 있을 수 있음
      loading.destroy();
      expect(loading.hideTimer).toBeNull();
    });
  });
});
